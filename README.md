...
**Стабильная версия — 1.2.2.** Для проверки новых возможностей доступна [тестовая 1.3.1](https://github.com/noginegun-web/navigator-map-scum-releases/releases/tag/v1.3.1-test). Продажи подписки пока не открыты.
...
![Компактная панель и фильтры мест интереса в тестовой версии](images/navigator-131-places.png)

Также улучшено открытие карты, убраны лишние перерисовки при её перемещении и добавлена отмена скачивания обновления. Исправлено восстановление активации при обрыве связи; неверное приглашение теперь не сбрасывает действующий доступ. Это тестовая сборка: длительная работа в самой SCUM ещё требует проверки.
...
| Выбрать бункеры, заправки и другие ориентиры в 1.3.1 | «Места интереса» → включите нужные категории |
| Отменить скачивание обновления в 1.3.1 | Нажмите «Отмена» на кнопке с процентом загрузки |
...
**Продажи пока закрыты. Отряды доступны в тестовой сборке по выданным тестовым ключам.** Подписка нужна каждому участнику. Автоматических повторных списаний нет. Счёт будет выставляться в рублях; доступность иностранных карт зависит от платёжного сервиса и банка. Приём реальных платежей ещё не подтверждён.
